...
describes what users get. Pick a format with `-o`, or let rtd choose:
a table when writing to a terminal, JSONL when piped.

## Formats

```bash
rtd <command> -o table   # aligned columns for reading
rtd <command> -o jsonl   # one JSON object per line, for piping
rtd <command> -o json    # a single JSON array
rtd <command> -o csv     # spreadsheet friendly
rtd <command> -o tsv     # tab-separated
rtd <command> -o url     # just the URL column
rtd <command> -o raw     # the underlying bytes, unformatted
```
...
```bash
rtd <command> --fields id,title,url
```
...
```bash
rtd <command> --template '{{.URL}} {{.Title}}'
```
...
```bash
rtd <command>            # a table, because this is a terminal
rtd <command> | wc -l    # JSONL, because this is a pipe
```
